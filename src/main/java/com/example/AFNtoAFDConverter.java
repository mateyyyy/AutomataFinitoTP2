...
public class AFNtoAFDConverter {
    ...
    public ArrayList<Integer> clausuraLambda(ArrayList<Integer> estadosIniciales, ArrayList<Integer> lista,
        ...
        for (int j = 0; j < estadosIniciales.size(); j++) {
            for (int i = 0; i < transiciones.size(); i++) {
                if (transiciones.get(i).getEstadoInicial() == estadosIniciales.get(j)
                        && transiciones.get(i).getCaracter() == '-') {
    ...
    public Automata AfnToAfd(Automata automata) {
        ...
        for (int i = 0; i < estados.get(0).getConjuntos().size(); i++) {
            clausuraLambda(Mover(estados.get(0).getConjuntos().get(i), 'a', automata.transiciones), estadosInicialesA,
                    automata.transiciones);
        }
        ArrayList<Integer> estadosInicialesB = new ArrayList<>();
        for (int i = 0; i < estados.get(0).getConjuntos().size(); i++) {
            clausuraLambda(Mover(estados.get(0).getConjuntos().get(i), 'b', automata.transiciones), estadosInicialesB,
                    automata.transiciones);
        }
